...
* **Profiles:** `keystash --profile work` (or `-P work`) keeps a fully separate vault — own database, own config, own git remote — under `~/.config/keystash/profiles/work/`. Works with every command and the TUI (the lock screen shows which profile is active); each profile syncs independently via `keystash --profile work sync setup`. Handy for `work`/`personal` splits, e.g. `alias kwork='keystash --profile work'`.
